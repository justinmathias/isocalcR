...
piru13C_long <- piru13C %>%
  dplyr::select(Year, Site, iWUE_simple, iWUE_photorespiration, iWUE_mesophyll) %>% #Select only columns of interest
  rename(Simple = iWUE_simple,
         Photorespiration = iWUE_photorespiration,
         Mesophyll = iWUE_mesophyll) %>% 
  tidyr::pivot_longer(names_to = "Formulation", values_to = "iWUE", -c(Year, Site))

#Visually examine differences in iWUE based on the formulation used for each study location
ggplot2::ggplot(data = piru13C_long, aes(x = Year, y = iWUE, color = Formulation)) +
